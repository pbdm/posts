...
* 如果请求含有 body, 则必须有 `Content-type`, 如果不发送, 服务器默认 `application/octet-stream`
* 如果是 GET 方法, 虽然可以发送 `Content-type`, 但是貌似并没有太大意义...
> [Do I need a content type for http get requests? in so](https://stackoverflow.com/questions/5661596/do-i-need-a-content-type-for-http-get-requests)
...
>
> [HTTP Content-type 与文件扩展名对照表](http://tool.oschina.net/commons)
...
* 适用于想用一个 http 请求多次接受数据的情况, 并不适用于只发送一次数据...(毕竟 http 连接一直存在)
